...
<cfcomponent extends="CFlickr.AbstractInterface">
	...
	<cffunction name="addComment" access="public" output="false" returntype="string" hint="Add comment to a photo as the currently authenticated user. returns the id of the new comment">
		<cfargument name="photo_id" type="string" required="yes" hint="The id of the photo to add a comment to.">
		<cfargument name="comment_text" type="string" required="yes" hint="Text of the comment.">
		
		<cfset var resp = flickr.get("flickr.photos.comments.addComment", arguments)>		
	...
	<cffunction name="deleteComment" access="public" output="false" returntype="void" hint="Delete a comment as the currently authenticated user.">
		...
		<cfset var resp = flickr.get("flickr.photos.comments.deleteComment", arguments)>		
		<cfif resp.isError()>
			<cfthrow errorcode="#resp.getErrorCode()#" message="#resp.getErrorMessage()#">
		</cfif>
	</cffunction>
	...
	<cffunction name="editComment" access="public" output="false" returntype="void" hint="Edit the text of a comment as the currently authenticated user.">
		...
		<cfset var resp = flickr.get("flickr.photos.comments.editComment", arguments)>		
		<cfif resp.isError()>
			<cfthrow errorcode="#resp.getErrorCode()#" message="#resp.getErrorMessage()#">
		</cfif>
	</cffunction>
